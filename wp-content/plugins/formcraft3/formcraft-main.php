  ...
  */
  ...
  class FormCraft_Plugin_Updater {
    ...
    public function setPluginInfo( $false, $action, $response ) {
      ...
      require_once( plugin_dir_path( __FILE__ ) . "php/Parser.lib.php" );
  ...
  function formcraft3_redirect_to_form_page()
  {
    ...
    function formcraft3_check()
    {
      ...
      if (is_user_logged_in() && isset($_GET['formcraft_export_entries']) && ctype_digit($_GET['formcraft_export_entries']) )
      {
        ...
        $meta = $meta['fields'];
        $data = $wpdb->get_results( "SELECT content, created FROM $fc_submissions_table WHERE form = '$form_id' LIMIT $exportFrom, $exportTo", ARRAY_A );
    ...
    function formcraft3_shortcode( $atts, $content = "" ) {
      ...
      if ( !empty($meta['config']['Messages']) )
      {
        ...
        wp_localize_script( 'fc-form-js', 'FC_Validation_'.$id,
  ...
  function formcraft3_file_upload()
  {
    ...
    if ( isset($_FILES['files']) )
    {
      foreach ($_FILES as $key => $file) {
        ...
        $allowed = array('png','doc','docx','xls','xlsx','csv','txt','rtf','zip','mp3','wma','wmv','mpg','flv','avi','jpg','jpeg','png','gif','ods','rar','ppt','pptx','tif','wav','mov','psd','eps','sit','sitx','cdr','ai','mp4','m4a','bmp','pps','aif', 'pdf', 'svg', 'odt','psa','stp','step','igs','x_t','dwg','obj','stl','bin','ols');
  ...
  */
  ...
  function fc_template($content, $body, $allow_html = false)
  {
    foreach ($content as $label => $value) {
      $value = nl2br($value);
      if ($allow_html == true) {
        $body = str_ireplace('['.$label.']', html_entity_decode(stripslashes($value), ENT_QUOTES, 'utf-8'), $body);
      } else {
